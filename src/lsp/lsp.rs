...
impl LanguageServer for Backend {
    ...
    async fn did_open(&self, params: DidOpenTextDocumentParams) {
        ...
        if self.state.ast_map.contains_key(&params.text_document.uri) {
            return;
        }
        ...
        let tree = self
            ...
            .write()
            .unwrap()
            .parse(params.text_document.text.clone())
            .expect("to parse file");
        print_tree(&tree);

        self.state
            .ast_map
            .insert(params.text_document.uri.clone(), tree);
        self.state
            .document_map
        ...
        while let Some(match_) = matches.next() {
                ...
                (namespace, dir, path)
            {
                if let (Ok(key_string), Ok(base_dir_string), Ok(path_string)) = (
                    ...
                    self.state.class_map.insert(namespace, full_path);
                }
            }
        }
    }
...
mod tests {
    ...
    async fn load_class_map() {
        ...
        assert_eq!(lang_server.state.class_map.is_empty(), false);
        assert_eq!(lang_server.state.class_map.iter().count(), 3);

        for (key, val) in lang_server.state.class_map.into_iter() {
